export const INDIAN_LANGS = new Set([
...
export function computeFinalScore(item: any, query: string, year_query: string | null) {
  let score = 0;
  ...
  else if (t1.split(' ').some((word: string) => word.includes(q)) || t2.split(' ').some((word: string) => word.includes(q))) {
    score += 40;
  }
  ...
  if (item.originalLanguage && INDIAN_LANGS.has(item.originalLanguage)) {
    score += 15;
  }
...
export function isIndianPerson(person: any) {
  const knownFor = Array.isArray(person?.known_for) ? person.known_for : [];
  return knownFor.some((item: any) => {
    ...
    const productionIso = productionCountries.map((c: any) => c?.iso_3166_1).filter(Boolean);
