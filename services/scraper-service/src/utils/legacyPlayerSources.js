...
const DEFAULT_MULTIMOVIES_BASE_URL = 'https://multimovies.wtf/';
const DEFAULT_MULTIMOVIES_ROOT_URL = 'https://multimovies.wtf/';
...
async function refreshPlayerSourceRecord(identity) {
  ...
  const onSource = async (sources = []) => {
    try {
      ...
      // Always write identity fields so cachedRecord is fully populated on next read
      const updateSet = {
        tmdbId: identity.tmdbId,
        imdbId: identity.imdbId || '',
        mediaType: identity.mediaType,
        ...
        searchKey: identity.mediaType === 'series' ? `series-master-${identity.tmdbId}` : (searchKey || ''),
...
async function resolveMultimoviesBaseUrlFromRoot(rootUrl) {
  // Plain fetch is enough for the root page — no Cloudflare JS challenge there
  ...
  // Also treat the final redirected URL as a candidate base URL
  const finalUrl = response.url || rootUrl;
  const baseUrl = extractMultimoviesBaseUrlFromRoot(html, finalUrl) || finalUrl;
  if (!baseUrl) {
...
async function scrapeWithMultimoviesConfig(identity, options = {}) {
  ...
  // Build candidate root URLs: stored list first, then known fallback domains
  const KNOWN_FALLBACK_ROOTS = [
    'https://multimovies.wtf/',
    'https://multimovies.fyi/',
    'https://multimoviesshg.com/',
    'https://multimovies.mom/',
    'https://multimovies.lol/',
    'https://multimovies.wiki/',
    'https://multimovies.zip/',
  ];
  const candidateRootUrls = [
    ...new Set([...(initialConfig.rootUrls || []), ...KNOWN_FALLBACK_ROOTS])
  ].filter(Boolean);

  let refreshedBaseUrl = null;
  let workingRootUrl = null;

  for (const rootUrl of candidateRootUrls) {
    try {
      console.log(`[Player Sources] Trying root URL for base refresh: ${rootUrl}`);
      const candidate = await resolveMultimoviesBaseUrlFromRoot(rootUrl);
      if (candidate) {
        refreshedBaseUrl = candidate;
        workingRootUrl = rootUrl;
        console.log(`[Player Sources] Root URL resolved! rootUrl=${rootUrl} → baseUrl=${refreshedBaseUrl}`);
        break;
      }
    } catch (err) {
      console.warn(`[Player Sources] Root URL failed: ${rootUrl} — ${err.message}`);
    }
  }

  if (refreshedBaseUrl && workingRootUrl) {
    await saveMultimoviesConfig({ rootUrl: workingRootUrl, baseUrl: refreshedBaseUrl, available: true });
    if (refreshedBaseUrl !== currentBaseUrl) {
      scrapeResult = await attemptScrape(refreshedBaseUrl, { onTableHit, onSource });
    }
  } else {
    console.error('[Player Sources] All root URL candidates exhausted — marking multimovies unavailable.');
  ...
  try {
    ...
    if (cachedRecord && !forceRefresh) {
      ...
      if (hasRealSources || (cachedRecord.notAvailable && isRecentlyAttempted) || isRecentlyAttempted) {
        // Check all possible lock key forms
        const storedKey = cachedRecord.searchKey || '';
        const masterKey = mediaType === 'series' ? `series-master-${tmdbId}` : '';
        const rebuiltKey = buildSearchKey({ mediaType, title: cachedRecord.title || '', year: cachedRecord.year || null, seasonNumber, episodeNumber });
        const epLockKey = mediaType === 'series' ? `series-${tmdbId}-s${seasonNumber}e${episodeNumber}` : '';
        let isScrapingNow = refreshLocks.has(storedKey) || refreshLocks.has(masterKey) || refreshLocks.has(rebuiltKey) || Boolean(epLockKey && refreshLocks.has(epLockKey));

        if (!isRecentlyAttempted && !isScrapingNow) {
          // Nothing scraping and record is stale — kick off a background refresh
          const fastIdentity = {
            mediaType,
            tmdbId,
            ...
            episodeTitle: cachedRecord.episodeTitle || '',
            directors: [],
            cast: []
          };
          // Only refresh if we have a title — blank identity scrapes are useless
          if (fastIdentity.title) {
            isScrapingNow = true;
            refreshPlayerSourceRecord(fastIdentity)
              .catch(err => console.error('[Player Sources] Background refresh failed:', err.message));
          }
        }


        console.log(`*********************** PlayerSources send ${payload.sources.filter(s => !s.isDirect).length} sources (Scraping: ${isScrapingNow}).`);
        return res.json({
          ...payload,
          cacheHit: true,
          scraping: isScrapingNow
        });
    ...
    if (cachedRecord && !forceRefresh) {
      ...
      if (hasRealSources || (cachedRecord.notAvailable && isRecentlyAttempted)) {
        ...
        console.log(`*********************** PlayerSources (by SearchKey) send ${payload.sources.filter(s => !s.isDirect).length} sources (Scraping: ${isScrapingNow}).`);
        return res.json({
          ...payload,
          cacheHit: true,
          scraping: isScrapingNow
